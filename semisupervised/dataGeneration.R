...
GenerateData<- function(n,N=0,p,option){
  ...
  if (option=="i")  
  {
    ...
    #eta=rnorm(n,0,sigma)
    Y_labelled=alpha0+X_labelled%*%alpha1+(X_labelled^3-X_labelled^2+exp(X_labelled))%*%alpha2+eta ###OM2
    ...
    if(N!=0){
      #X_unlabelled=mvrnorm(N,rep(0,p),diag(rep(1,p)))  # unlabelled data set
      X_unlabelled=X[D==0,]
