...
newton_ET_primal_test <- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 50, eps = 1e-6)
{
  ...
  while (TRUE) {
    ...
    #H1<-apply(new.data1,2,function(a)((y.hat)*a))
    #H2<-apply(new.data1[,-1],2,function(a)(a*a))
    
    #H<-cbind(H1)
    H<-cbind(H1,log(pi.hat))
    R=H[1:nrow(data_labelled),]
    p<-sum(nrow(data_labelled))
...
newton_HD_primal<- function(y.hat,data_labelled,data_unlabelled,n,N)
{
    ...
    ###Primal
    new.data<-(rbind(data_labelled[,-1],data_unlabelled))
    new.data1<-cbind(new.data)
    
    
    
    H1<-apply(new.data1,2,function(a)(a*y.hat))
    H2<-apply(new.data1,2,function(a)(a*a))
...
newton_HD_primal_test <- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 200, eps = 1e-4)
{
...
newton_square_primal<- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 200, eps = 1e-4)
{
  ...
  while (TRUE) {
    ...
    H<-apply(new.data1,2,function(a)((y.hat-as.numeric(new.data1%*%theta))*a))
    
    dat.new<-cbind(D=c(rep(1,nrow(data_labelled[,-1])),rep(0,nrow(data_unlabelled))),new.data)
    pi.hat<-fitted(glm(D~.,data=as.data.frame(dat.new),family=binomial()))
...
newton_HD_primal_test <- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 200, eps = 1e-6)
{
...
newton_HD_primal_test111<- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 200, eps = 1e-6)
{
...
newton_ET_supervised <- function(th, y.hat,data_labelled,data_unlabelled,n,N, max.iter = 200, eps = 1e-6)
{
  ...
  while (TRUE) {
    ...
    new.data<-(rbind(data_labelled[,-1],data_unlabelled))
    new.data1<-cbind(1,new.data)
    new.data2<-cbind(1,data_labelled[,-1])
    error<-y.hat-as.numeric(new.data1%*%theta)
    #H1<-apply(new.data1,2,function(a)((y.hat)*a))
    #H2<-apply(new.data2,2,function(a)((data_labelled[,1]-as.numeric(new.data2%*%theta))*a))
    
    H2<-apply(new.data2,2,function(a)((data_labelled[,1]))*a)
    H3<-apply(cbind(1,new.data1),2,function(a)(a*y.hat))
    H4<-apply(new.data1,2,function(a)(a*a))
    
    H1<-cbind(H4)
    
    R1=H1[1:nrow(data_labelled),]
    R2=H2
    p<-sum(nrow(data_labelled))
    ...
    A <- cbind(rep(1, n), R1,R2)
    b <-c(1, colSums(H1) / (n + N),rep(0,ncol(H2)))
...
newton_ET_primal<- function( y.hat,data_labelled,data_unlabelled,n,N)
{
  new.data<-(rbind(data_labelled[,-1],data_unlabelled))
  new.data1<-cbind(new.data)
  
  H1<-apply(cbind(1,new.data1),2,function(a)(a*y.hat))
  H2<-apply(new.data1,2,function(a)(a*a))
...
newton_square_Z<- function(labelled_Z,unlabelled_Z,data_labelled,n,N)
{
  
  
   
    H<-rbind(labelled_Z,unlabelled_Z)
    R=H[1:nrow(labelled_Z),]
    p<-sum(nrow(labelled_Z))
...
y.hat.p7<-function(folds,q){
 
  predL_full<-matrix(nrow = nrow(data_labelled), ncol =1)
  predU_full<-matrix(nrow = nrow(data_unlabelled), ncol =q)
  for(k in 1:q){
    test_data <- data_labelled[folds[[k]],]
    train_data <- data_labelled[-folds[[k]],]
    
    
    
    model<-gam(Y ~ s(U1,bs="tp")+s(U2,bs="tp")+s(U3,bs="tp")+s(U4,bs="tp")+s(U5,bs="tp")+s(U6,bs="tp")+s(U7,bs="tp"),data=as.data.frame(train_data),method = "REML")
    predL <- predict(model, newdata=as.data.frame(test_data[,-1]))
    predU<-as.numeric(predict(model, newdata=as.data.frame(data_unlabelled)))
    
    predL_full[folds[[k]], ] <- predL
    
    predU_full[,k]<-predU
   
  }
  
  y.hatU<-as.numeric(apply(predU_full,1,function(a)mean(a)))
  
  #y.hat<-c(predL_full,y.hatU)
  y.hat<-c(predL_full,y.hatU)
...
y.hat.p4<-function(folds,q){
  ...
  for(k in 1:q){
    ...
    model<-gam(Y ~ s(U1,bs="tp")+s(U2,bs="tp")+s(U3,bs="tp")+s(U4,bs="tp"),data=as.data.frame(train_data),method = "REML")
    predL <- predict(model, newdata=as.data.frame(test_data[,-1]))
    predU<-as.numeric(predict(model, newdata=as.data.frame(data_unlabelled)))
