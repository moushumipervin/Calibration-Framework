...
U_fn <- function(theta, data_labelled,data_unlabelled,y.hat) {
  ...
  X_all <- as.matrix(rbind(data_labelled[, -1], data_unlabelled))  # N x p
...
solve_lambda <- function(theta,data_labelled,data_unlabelled,y.hat,
  ...
  mu_target<-c( c(colMeans(H),rep(0,ncol(data_unlabelled))))
...
#solve_lambda(theta,data_labelled,data_unlabelled,y.hat)$lamda_hat1 
...
estimate_theta <- function(theta, data_labelled,data_unlabelled,y.hat) {
  ...
  obj_fn <- function(theta) {
    ...
    return( -(m+log(sum(exp(eta-m))))+(as.vector(U1 %*% lambda_hat[1:(p+1)]))) 
  }
...
make_profile_with_Hmeans <- function(data_labelled, data_unlabelled, y.hat) {
  ...
  L_fn <- function(theta) {
    ...
    U1<-c(colMeans(H),rep(0,ncol(data_unlabelled)))
    
    return( -log(sum(exp(eta)))+(as.vector(U1 %*% lambda_hat))) 
  }
...
newton_ET_primal_test <- function(th, y.hat, data_labelled, data_unlabelled, 
