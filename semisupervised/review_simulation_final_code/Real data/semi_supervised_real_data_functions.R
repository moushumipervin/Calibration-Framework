...
estimate_theta_EM_kfold_CVXR_ET <- function(th, data_full, K, seed, max.iter,eps) {
  ...
  repeat {
    ...
    error_hat <- y.hat - as.numeric(new.data1 %*% theta)
    # H: N x (p+1)
    b_mat <- new.data1 * error_hat
    # Moment matrix for calibration (choose what you want to balance)
    
    g_pi <- log(1 / data_all$pi.hat)
    
    w <- CVXR::Variable(length(I1), pos = TRUE)
    
    b_resp <- b_mat[I1, , drop = FALSE]
    ...
    lambda1_hat <- as.numeric(
      CVXR::dual_value(constr_balance)
    )
    
    lambda2_hat <- as.numeric(
      CVXR::dual_value(constr_debias)
    )
    ...
    if (max(abs(th.new - as.numeric(theta))) < eps) {
      ...
      return(
        list(
          ...
          data_all = data_all,
          b_mat = b_mat,
          g_pi = g_pi,estimate.table=variance_theta_diag(
            y = y,
            X = new.data1[I1, ],
            w_hat = W,
            theta_hat = th.new
...
estimate_theta_EM_kfold_CVXR_HD <- function(
    th, data_full, K, seed, max.iter, eps) {
  ...
  x_cols <- subset(
    data_all,
    select = -c(Y, D, pi.hat, ID, y.hat)
  )
  
  model.formula <- as.formula(
    paste(
      "~",
      paste(colnames(x_cols), collapse = "+")
    )
  )
  
  new.data1 <- model.matrix(
    model.formula,
    data = as.data.frame(
      data_all[, colnames(x_cols), drop = FALSE]
    )
  )
  ...
  repeat {
    ...
    # ----------------------------------------------------------
    # Cross-fitted calibration function b_i(theta)
    # ----------------------------------------------------------
    error_hat <- y.hat -
      as.numeric(new.data1 %*% theta)
    
    b_mat <- new.data1 * error_hat
    
    
    # ----------------------------------------------------------
    # HD debiasing covariate:
    #
    # g(w) = -1 / (2 sqrt(w))
    #
    # therefore
    ...
    # g(pi^{-1}) = -sqrt(pi) / 2
    # ----------------------------------------------------------
    g_pi <- -sqrt(data_all$pi.hat) / 2
    
    
    # ----------------------------------------------------------
    # CVXR weights for respondents only
    # ----------------------------------------------------------
    w <- CVXR::Variable(
      length(I1),
      pos = TRUE
    ...
    # ----------------------------------------------------------
    # Global calibration balance
    #
    # sum D_i w_i b_i = sum b_i
    # ----------------------------------------------------------
    constr_balance <-
      t(b_resp) %*% w ==
      matrix(
    ...
    # ----------------------------------------------------------
    # Global debiasing constraint
    #
    # sum D_i w_i g(pi_i^{-1}) = sum g(pi_i^{-1})
    # ----------------------------------------------------------
    constr_debias <-
      sum(
        w * g_pi[I1]
...
estimate_theta_EM_kfold_CVXR_CE <- function(
    th, data_full, K, seed, max.iter, eps) {
  ...
  repeat {
    ...
    # ----------------------------------------------------------
    # Cross-fitted calibration function
    #
    # b_i(theta)
    ...
    # ----------------------------------------------------------
    # Contrast entropy debiasing covariate
    #
    # g(pi^{-1}) = log(1 - pi)
    # ----------------------------------------------------------
    ...
    # CE domain
    #
    # G(w) is defined for w > 1.
    #
    # Numerically keep slightly away from boundary.
    # ----------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Contrast entropy objective
    #
    # G(w)
    # =
    # (w-1)log(w-1) - w log(w)
    #
    # Equivalent CVXR-friendly form:
    #
    # rel_entr(w-1, w) - log(w)
    # ----------------------------------------------------------
    ...
    # CLARABEL is a good choice here.
    ...
    # ==========================================================
    # 5. CVXR dual multipliers
    # ==========================================================
    
    lambda1_cvxr <- as.numeric(
      CVXR::dual_value(
        constr_balance
      )
    )
    
    lambda2_cvxr <- as.numeric(
      CVXR::dual_value(
        constr_debias
      )
    )
    ...
    if (diff_theta < eps) {
      ...
      return(
        list(
          ...
          # lambda according to PAPER convention
          lambda = lambda_hat,
